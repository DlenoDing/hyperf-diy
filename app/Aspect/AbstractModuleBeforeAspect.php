...
/**
 * 模块前置切面基类：封装与模块无关的「签名校验 + 数据解密」公共逻辑。
 * Admin / Api 子类各自负责：
 *   - isMatch()   ：判定当前请求是否由本切面处理（后台模块 / 非后台模块）
 *   - checkAuth() ：实现各自不同的登录态校验（后台登录 vs API 端 token，逻辑必然不同）
 *
 * 注意：本类为抽象类，不加 #[Aspect]，仅供子类继承；
 * 子类继承的 public $classes/$annotations 会被 Hyperf AspectLoader 正常读取（getProperties 含继承属性）。
 */
abstract class AbstractModuleBeforeAspect extends AbstractAspect
{
    ...
    /**
     * 检查登录状态（子类各自实现：后台登录 / API 端 token，逻辑必然不同）
     * @param $whiteVal
     */

 ...
 *
 * 作用：声明「延时缓冲队列」AppExampleDelayDlxBufferQueue，给它配上
 *   - x-message-ttl     = messageTtl 秒（消息在此滞留的延时时长）
 *   - x-dead-letter-exchange / -routing-key = 死信交换机/路由（到点后转投目标）
 * 生产者 {@see \App\Amqp\Producer\DelayDlxProducer} 投进来的消息在此滞留 TTL 后过期，
 ...
 */
#[Consumer(exchange: 'AppExampleDelayDlxBufferExchange', routingKey: 'AppExampleDelayDlxBufferRouting', queue: 'AppExampleDelayDlxBufferQueue', name: 'AppExampleDelayDlxBufferConsumer', nums: 1)]
class DelayDlxBufferConsumer extends BaseConsumer
{
    protected string $poolName = 'consumer';

    protected string $exchange = 'AppExampleDelayDlxBufferExchange';

    protected array|string $routingKey = 'AppExampleDelayDlxBufferRouting';

    protected ?string $queue = 'AppExampleDelayDlxBufferQueue';

    /**
     * 死信交换机/路由：消息过期后转投到此（即下方 DelayDlxDeadConsumer 监听的交换机/路由）。
     */
    protected $deadExchange = 'AppExampleDelayDlxDeadExchange';

    protected $deadRoutingKey = 'AppExampleDelayDlxDeadRouting';
